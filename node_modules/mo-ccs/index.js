 ...
 * @returns {boolean}
 */
exports.isCca = function(ccaPath){
 ...
 * @param ccaPath
 * @param scale
 * @param {{outDir:String, outName:String, uglifyJsonOpt:{JSON_KEY:Object, JSON_DEFAULT_VALUE:Object}, isJsonExcluded:Boolean, isResExcluded:Boolean}} opt
 * @param cb
 * @returns {*}
 */
exports.publishCca = function(ccaPath, scale, opt, cb){
    if(!exports.isCca(ccaPath)) return cb();//不是cca
    async.parallel([
        function(cb1){
            if(opt.isJsonExcluded) return cb1();
            exports.publishCcaJson(ccaPath, scale, opt, cb1);
        },
        function(cb1){
            if(opt.isResExcluded) return cb1();
            exports.publishCcaRes(ccaPath, scale, opt, cb1);
        }
    ], cb);
};
/**
 ...
 * @returns {*}
 */
exports.publishCcaRes = function(ccaPath, scale, opt, cb){
    if(!exports.isCca(ccaPath)) return cb();//不是cca
 ...
 * @returns {*}
 */
exports.publishCcaJson = function(ccaPath, scale, opt, cb){
    if(!exports.isCca(ccaPath)) return cb();//不是cca
    var ccaName = path.basename(ccaPath);//获取cca工程名称
    opt = opt || {};//选项
    var outDir = opt.outDir || path.join(ccaPath, "output");
...
function isPreviewNode(jsonData, delModules, scatterModules){
    if(jsonData.classname == "LabelAtlas"){
        var fd = jsonData.options.charMapFileData;
        var pathStr = fd.path;
        var module = pathStr.split("/")[0];
        if(delModules.indexOf(module) >= 0) return true;
        fd.resourceType = scatterModules.indexOf(module) >= 0 ? 2 : 0;
        return false;
    }else if(jsonData.classname == "ImageView") {
        var fd = jsonData.options.fileNameData;
        var pathStr = fd.path;
        var module = pathStr.split("/")[0];
        if(delModules.indexOf(module) >= 0) return true;
        fd.resourceType = scatterModules.indexOf(module) >= 0 ? 2 : 1;
    }
    return false;
}
...
//exports.publishCcuiJson(path.join(__dirname, "../../__test/ccui/Json/uiVipLayer.json"));
