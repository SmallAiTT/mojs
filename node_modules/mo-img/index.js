...
var g_pluginsPath = path.join(__dirname, "plugins");
var g_pyDir = path.join(g_pluginsPath, "py");

var g_imgOpt = "RGBA5555";

exports.packedMap = {};//已经打过包的列表，避免重复打包。

exports.config = function(cfg){
    g_imgOpt = cfg.imgOpt || "RGBA5555";
};
...
exports.pngquant = function(options, cb){
    ...
    args.splice(0, 0, path.join(g_pluginsPath, "pngquant/pngquant"));
...
exports.texturePacker = function(assets, outDir, scale, name, cb){
    ...
    moUtils.cmd("TexturePacker", assets, {"--scale" : scale, "--data" : o_plist, "--sheet" : o_png, "--opt" : g_imgOpt, "--trim-mode" : "Trim", "--trim-threshold" : 10}, cb)
};
...
exports.packImg = function(assets, outDir, scale, name, tempDir, isWithDirName, cb){
    ...
    exports.scaleImgs(assets, isWithDirName ? path.join(scaleDir, assetsName) : scaleDir, scale, function(err){
        ...
        //接着进行打包
        exports.texturePacker(scaleDir, outDir, 1, name, cb);
    });
};
...
exports.scaleImg = function(srcPath, dstPath, scale, cb){
    ...
    var subDstDir = path.dirname(dstPath);
    moUtils.mkdirSync(subDstDir);
    moUtils.py(path.join(g_pyDir, "scaleImg.py"), srcPath, dstPath, scale, cb);
};
exports.scaleImgWithQuality = function(srcPath, dstPath, scale, cb){
    exports.scaleImg(srcPath, dstPath, scale, function(err){
        if(err) return cb(err);
        if(moUtils.isExtname(srcPath, ".png")){
            exports.pngquant(dstPath, {"--output" : dstPath}, "--force", cb);
        }else{
            cb();
        }
    });
...
exports.scaleImgs = function(srcDir, dstDir, scale, cb){
    ...
    moUtils.walkDirOneByOne(srcDir, [".png", ".jpg"], function(filePath, cb1){
        var dstFilePath = path.join(dstDir, path.relative(srcDir, filePath));
        exports.scaleImgWithQuality(filePath, dstFilePath, scale, cb1);
    }, cb);
};
...
exports.scaleImgsToOneDir = function(srcDir, dstDir, scale, cb){
    moUtils.mkdirSync(dstDir);
    moUtils.walkDirOneByOne(srcDir, [".png", ".jpg"], function(filePath, cb1){
        var dstFilePath = path.join(dstDir, path.basename(filePath));
        exports.scaleImgWithQuality(filePath, dstFilePath, scale, cb1);
    }, cb);
}
...
exports.repack = function(plist, imgDir, cb){
    ...
    var pyPath = path.join(g_pyDir, "repacker.py")
...
exports.pack = function(imgDir, scale, outDir, outName, tempDir, cb){
    ...
    exports.scaleImgs(imgDir, __resTempDir, scale, function(err){
        if(err) return cb(err);
        exports.texturePacker(__resourcesTempDir, outDir, 1, outName, cb);
    });
}