...
exports.scatteredArr = ["preview", "ui_bg", "ui_num"];
exports.uglified = false;
...
//        console.log(value, "--->", value.substring(0, index+1) + num);
    ...
    var newData = JSON.parse(content);//再次转换成对象
    require("./defaultValueHandler.js").handle(newData, uiKey);
    content = JSON.stringify(newData, null, 0);
    return content;
}

exports.convert = function(jsonPath, outPath){
    uiKey = exports.uglified ? require("./key_uglified.js") : require("./key.js");
...
function delPreview(parent, lvl){
    lvl = lvl || 1;
    var children = parent[uiKey.children] ||[];
...
function handleUIInput(data, result, textures){
    ...
    result[uiKey.placeHolder] = data["placeHolder"];
}
...
function handleUILoadingBar(data, result, textures){
    var resPath;
    result[uiKey.percent] = data["percent"];
    if(data["textureData"]) {
...
function trainsform(widgetData, parent){
    if(parent) {
        var pW = parent[uiKey.innerWidth] || parent[uiKey.width] || 0;
        var pH = parent[uiKey.innerHeight] || parent[uiKey.height] || 0;
...
function handleResPath(filePath, textures){
    ...
    if(!exports.isScatterMode && exports.scatteredArr.indexOf(pre) < 0){
        var extname = path.extname(filePath);
        var fn = filePath.substring(index+1);
//        fn = path.basename(fn, extname)
        filePath = pre + "#" + fn;
        var sheet = pre+".sheet";
