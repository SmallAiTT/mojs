...
function handleUIRoot(data, result){
    ...
    var textures = result[uiKey.textures] = data["textures"];
    result[uiKey.dataScale] = data["dataScale"];
    result[uiKey.widgetTree] = convertWidgetData(data["widgetTree"], textures);
    trainsform(result[uiKey.widgetTree], null);
}
...
function handleUIPanel(data, result, textures){
    ...
    result[uiKey.capInsetsX] = data["capInsetsX"];
    result[uiKey.capInsetsY] = data["capInsetsY"];
    result[uiKey.capInsetsHeight] = data["capInsetsHeight"];
    result[uiKey.capInsetsWidth] = data["capInsetsWidth"];
    result[uiKey.clipAble] = data["clipAble"];
    result[uiKey.layoutType] = data["layoutType"];
    result[uiKey.vectorX] = data["vectorX"];
    result[uiKey.vectorY] = data["vectorY"];
    if(data["backGroundImageData"]) result[uiKey.res] = data["backGroundImageData"]["path"]
...
function handleUITextAtlas(data, result, textures){
    ...
    result[uiKey.itemHeight] = data["uiKey"];
...
function handleUIImage(data, result, textures){
    result[uiKey.res] = data["fileNameData"]["path"];
}

function handleUIButton(data, result, textures){
    if(data["normalData"]) result[uiKey.normal] = data["normalData"]["path"];
    if(data["pressedData"]) result[uiKey.pressed] = data["pressedData"]["path"];
    if(data["disabledData"]) result[uiKey.disabled] = data["disabledData"]["path"];
    result[uiKey.text] = data["text"];
...
function trainsform(widgetData, parent){
    if(parent) {
        ...
        widgetData[uiKey.y] = pH - y - h * (1 - 2 * ay);
    }
...
function test(){
    var ccuiDir = path.join(__dirname, "example/ccui");
    var mouiDir = path.join(__dirname, "example/moui");
    exports.convertByDir(ccuiDir, mouiDir);
}
//test();